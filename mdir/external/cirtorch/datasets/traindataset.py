...
class TuplesDataset(data.Dataset):
    ...
    def __init__(self, name, mode, imsize=None, nnum=5, qsize=2000, poolsize=20000, transform=None,
                 loader=default_loader, dataset_pkl=None, ims_root=None):
    ...
    def create_epoch_tuples(self, net, device=None):
        ...
        with torch.no_grad():
            ...
            qvecs = torch.zeros(net.meta['out_channels'], len(self.qidxs)).to(device)
            for i, input in enumerate(loader):
                qvecs[:, i] = net(input.to(device)).data.squeeze()
            ...
            poolvecs = torch.zeros(net.meta['out_channels'], len(idxs2images)).to(device)
            for i, input in enumerate(loader):
                poolvecs[:, i] = net(input.to(device)).data.squeeze()
            ...
            avg_ndist = torch.tensor(0).float().to(device)  # for statistics
            n_ndist = torch.tensor(0).float().to(device)  # for statistics
            ndist_acc = []
        ...
        return {"average_negative_distance": ndist_acc}  # return average negative l2-distance
