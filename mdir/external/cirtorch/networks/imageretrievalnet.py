...
def init_network(params):
    ...
    if whitening:
        ...
        if isinstance(whitening, str):
            with open(whitening, 'rb') as handle:
                whit_loaded = pickle.load(handle)
                whiten.load_state_dict({"weight": torch.tensor(whit_loaded['P']),
                                        "bias": -torch.mm(torch.tensor(whit_loaded['P']), torch.tensor(whit_loaded['m'])).squeeze()})
        elif pretrained:
            w = architecture
            if local_whitening:
                w += '-lw'
    ...
    if pretrained and architecture in FEATURES:
        ...
        model_dir = model_dir or os.path.join(get_data_root(), 'networks')
        net.features.load_state_dict(model_zoo.load_url(FEATURES[architecture], model_dir=model_dir))

    return net


def extract_vectors(net, images, image_size, transform, bbxs=None, ms=[1], msp=1, print_freq=10, device=None):
    ...
    # extracting vectors
    with torch.no_grad():
        vecs = torch.zeros(net.meta['out_channels'], len(images))
        for i, input in enumerate(loader):
            input = input.cuda() if not device else input.to(device)
...
def extract_ms(net, input, ms, msp):
    ...
    v = torch.zeros(net.meta['out_channels'])
...
def extract_ssl(net, input):
    return net.norm(net.features(input)).squeeze(0).view(net.meta['out_channels'], -1).cpu().data
